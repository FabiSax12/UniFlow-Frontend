...
const DashboardTaskCard = ({ task, className = "" }: Props) => {
        ...
        <div className='mb-2 '>
          <div className={`flex items-start justify-between mb-1 ${className}`}>
            ...
            <div className='flex gap-3'>
              {/* Status Badge */}
              <Badge variant={statusStyles.badgeVariant} className={`text-base ml-2 ${className}`}>
                {task.status.toUpperCase()}
              </Badge>
              {/* Priority Badge con colores dinámicos */}
              <Badge
                variant={priorityStyles.badgeVariant}
                className={`text-base ${priorityStyles.customClasses} ${className}`}
              >
